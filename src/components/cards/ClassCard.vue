      ...
      <div class="class-row">
        <div style>
          <div class="name-col">
            <div
              class="class-name"
              style="position:relative; top:7px; transition: all 0.15s ease-in-out"
              :style="{'text-decoration': done ? 'line-through':'none', 'color': done ? 'gray': (light ? '#2c3e50' : 'white')}"
            >{{course.name}}</div>
            <div
              class
              style="padding-top:10px; padding-left:2px; color: #BDC3C7; font-size:13px; "
            >{{course.hour}}:{{course.minute > 10 ? '': '0'}}{{course.minute}} {{course.isAm ? 'AM' : 'PM'}}</div>
          </div>
        </div>
        ...
        <div class="info fadeIn" style="height:55px;">
          <div
            class
            style="display:flex; padding-top: 7px; align-items: center; align-content: center; align-content: center;"
          >
            ...
            >
              ...
              >Launch</div>
              <font-awesome-icon
                :style="{'transform' : hover ? 'translatex(4px) scale(1.1)':''}"
